...
export function evaluateGroups(input, data = dataset) {
  ...
  for (const groupId of GROUP_KEYS) {
    ...
    if (!allowed) {
      ...
      detail.trace.push('Tabla 1.5.3: grupo no permitido.');
      result[groupId] = detail;
      continue;
    }

    detail.trace.push('Tabla 1.5.3: grupo permitido en la fila seleccionada.');
...
export default function evaluateLRNavalShips(input, data = dataset) {
  const detail = cloneDetail();
  const { joint } = input || {};
  const system = findSystem(input?.systemId, data);
  ...
  if (!rule.classes.includes(pipeClass)) {
    detail.status = 'forbidden';
    detail.reasons.push(`Tabla 1.5.4: Clase ${pipeClass} no aplicable a ${rule.name}.`);
  ...
  if (typeof limit === 'number' && Number.isFinite(od) && od > limit) {
    detail.status = 'forbidden';
    detail.reasons.push(`Tabla 1.5.4: OD ${od.toFixed(1)} mm excede ${limit} mm para la clase ${pipeClass}.`);
    detail.trace.push('Diámetro exterior supera el límite permitido.');
    return detail;
  }

  if (config.group === 'slip_on_joints') {
    const clause5109 = '5.10.9';
    const restrictedSpaces = ['cargo_hold', 'tank', 'munitions_store'];
    if (restrictedSpaces.includes(input.space) || input.accessibility === 'not_easy') {
      detail.status = 'forbidden';
      detail.reasons.push(data.clauses?.[clause5109] || 'Slip-on no permitidas en bodegas, tanques o espacios no accesibles.');
      addClause(detail, clause5109, data);
      detail.trace.push('Restricción de ubicación aplicada (5.10.9).');
      return detail;
    }

    if (input.asMainMeans) {
      const clause5110 = '5.10.10';
      detail.status = 'forbidden';
      detail.reasons.push(data.clauses?.[clause5110] || 'Slip-type no puede utilizarse como medio principal de unión.');
      addClause(detail, clause5110, data);
      detail.trace.push('Restricción por uso como medio principal (5.10.10).');
      return detail;
    }

    if (system.id === 'steam' && input.space === 'open_deck') {
      const clause5111 = '5.10.11';
      if ((input.designPressure_bar ?? 0) <= 10) {
        detail.status = 'conditional';
        detail.conditions.push(data.clauses?.[clause5111] || 'Slip-on restringidos en líneas de vapor ≤10 bar en cubierta expuesta.');
        addClause(detail, clause5111, data);
        detail.trace.push('Condición de presión aplicada para vapor en cubierta.');
      } else {
        detail.status = 'forbidden';
        detail.reasons.push(data.clauses?.[clause5111] || 'Condiciones de vapor en cubierta no cumplen los requisitos.');
        addClause(detail, clause5111, data);
